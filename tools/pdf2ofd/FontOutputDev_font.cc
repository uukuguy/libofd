...
std::string FontOutputDev::dumpEmbeddedFont(GfxFont * font, FontInfo & info) {
    ...
    try {
        ...
        if(!outf){
            throw std::string("Cannot open file ") + filepath + " for writing";
        }
...
std::string FontOutputDev::dumpType3Font(GfxFont *font, FontInfo & info) {
    ...
    */
    ...
    for(int code = 0; code < 256; ++code)
    {
...

        {
            ...
            cairo_destroy(cr);
            if(status)
                throw std::string("Cairo error: ") + cairo_status_to_string(status);
        }
        cairo_surface_finish(surface);
        {
            ...
            surface = nullptr;
            if(status)
                throw std::string("Error in cairo: ") + cairo_status_to_string(status);
        }
    ...
    std::string font_filename = (char*)str_fmt("%s/f%llx.ttf", param.tmp_dir.c_str(), fn_id);
...
void FontOutputDev::embedFont(const std::string & filepath, GfxFont * font, FontInfo & info, bool get_metric_only) {
    ...
    if(font_8bit) {
        ...
        if(is_truetype_suffix(suffix))
        {
            ...
            for(int i = 0; i < 256; ++i)
            {
                ...
                {
                    if(nameset.insert(std::string(cn)).second)
                    {
                        cur_mapping2[i] = cn;    
                    }
                    else
...

    {
        ...
        if(code2GID)
            maxcode = std::min<int>(maxcode, code2GID_len - 1);
...
const FontInfo * FontOutputDev::installFont(GfxFont * font) {
...
void FontOutputDev::installEmbeddedFont(GfxFont * font, FontInfo & info) {
    auto path = dumpEmbeddedFont(font, info);

    if(path != "") {
        embedFont(path, font, info);
...
const std::map<std::string, std::string> GB_ENCODED_FONT_NAME_MAP_1({
...
void FontOutputDev::installExternalFont(GfxFont * font, FontInfo & info){
    ...
    if(localfontloc != nullptr) {
        ...
        embedFont(std::string(localfontloc->path->getCString()), font, info, true);
        delete localfontloc;
    } else {
...
//void FontOutputDev::export_remote_font(const FontInfo & info, const std::string & format, GfxFont * font) {
    //std::string css_font_format;
    //if(format == "ttf")
    //{
    ...
    //else
    //{
        //throw std::string("Warning: unknown font format: ") + format;
    //}
    ...
    //{
        //throw std::string("Warning: unknown font format: ") + format;
    //}
    //std::string mime_type = iter->second;
...
//void FontOutputDev::export_remote_default_font(long long fn_id) {
    //f_css.fs << "." << CSS::FONT_FAMILY_CN << fn_id << "{font-family:sans-serif;visibility:hidden;}" << endl;
//}

//void FontOutputDev::export_local_font(const FontInfo & info, GfxFont * font, const std::string & original_font_name, const std::string & cssfont) {
    //f_css.fs << "." << CSS::FONT_FAMILY_CN << info.id << "{";
    //f_css.fs << "font-family:" << ((cssfont == "") ? (original_font_name + "," + general_font_family(font)) : cssfont) << ";";

    //std::string fn = original_font_name;
    //for(auto & c : fn)
        //c = tolower(c);

    //if(font->isBold() || (fn.find("bold") != std::string::npos))
        //f_css.fs << "font-weight:bold;";
    //else
        //f_css.fs << "font-weight:normal;";

    //if(fn.find("oblique") != std::string::npos)
        //f_css.fs << "font-style:oblique;";
    //else if(font->isItalic() || (fn.find("italic") != std::string::npos))
